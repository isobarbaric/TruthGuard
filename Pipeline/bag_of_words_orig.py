...
#         self.words = BagOfWords_rev.tokenize(self.article_texts)
#         BagOfWords_rev.to_lower_case(self.words)
#         BagOfWords_rev.clean_data(self.words)
#         BagOfWords_rev.remove_stop_words(self.words)
#         self.freq_chart = BagOfWords_rev.create_frequency_chart(self.words)
#         BagOfWords_rev.normalize_words(self.words) # TODO: normalize seems to be the wrong term here, I mean 'lemmatize', but also review what normalization is just in case

#         # only plotting the frequency when a distinguishable name is provided
#         if name is not None:
#             BagOfWords_rev.plot_frequency_chart(name, self.freq_chart)
